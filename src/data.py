...
def prepare_nn_dataset(
    ...
    if temporal_context:
        if 'ball_trajectory_vector' in events_df.columns:
...
class ContextPitchDatasetMultiTaskBallVector(Dataset):
    def __init__(self, df):
        self.df = df
        # Extract grids from the list-columns
        self.ball_layers = np.stack(df['ball_layer'].values)
        self.team_layers = np.stack(df['teammates_layer'].values)
        self.opp_layers = np.stack(df['opponents_layer'].values)
        
        # Extract the new Contextual Ball Trajectory
        self.context_data = np.stack(df['ball_trajectory_vector'].values).astype(np.float32)
        
        # Targets
        self.event_targets = torch.tensor(df['nn_target_idx'].values, dtype=torch.long)
    ...
    def __getitem__(self, idx):
        # Stack the 3 grid layers into a single [3, 12, 8] tensor
        grid = np.stack([
